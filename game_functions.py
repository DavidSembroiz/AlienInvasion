...
def check_events(ai_settings, stats, screen, sb, play_button, ship, aliens, 
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_play_button(ai_settings, stats, screen, sb, play_button,
...
def get_number_rows(ai_settings, ship_height, alien_height):
    ...
    number_rows = min(3, int(available_space_y / (2 * alien_height)))
    return number_rows
...
def create_alien(ai_settings, screen, ship, aliens, alien_number, row_number):
    ...
    alien.rect.y = 40 + ship.rect.height + 2 * alien.rect.height * row_number
    aliens.add(alien)
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    for row_number in range(number_rows):
        for alien_number in range(number_aliens_x):
            create_alien(ai_settings, screen, ship, aliens,
...
def update_bullets(ai_settings, stats, screen, sb, ship, aliens, bullets):
    ...
    check_bullet_alien_collisions(ai_settings, stats, screen, sb, ship, aliens,
        bullets)
    

def check_bullet_alien_collisions(ai_settings, stats, screen, sb, ship, aliens,
    bullets):
...
def check_fleet_edges(ai_settings, aliens):
    """Respond appropriately if any alien has reached the edges."""
    for alien in aliens.sprites():
